        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {loading ? (
              ...
              <tr><td colSpan="7" className="p-4">No data</td></tr>
            ) : (
              rows.map((r) => (
                <tr key={r.userId} className = "border-b border-gray-300">
                  <td className="px-4 py-3 font-medium">{r.name}</td>
                  <td className="px-4 py-3">{r.punchInTime ? new Date(r.punchInTime).toLocaleTimeString() : "—"}</td>
                  <td className="px-4 py-3">{r.punchOutTime ? new Date(r.punchOutTime).toLocaleTimeString() : "—"}</td>
